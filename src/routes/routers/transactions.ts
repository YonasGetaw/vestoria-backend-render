...
transactionsRouter.post("/send", requireAuth, async (req, res) => {
  ...
  if (!receiver.isActive) return res.status(400).json({ message: "Recipient account is not active" });

  if (!sender.withdrawPasswordHash) return res.status(400).json({ message: "Withdraw password is required to send money" });
